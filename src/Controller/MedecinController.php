...
class MedecinController extends AbstractController
{
    ...
    #[Route('/cloche', name: 'medecin_search', methods: ['GET'])]
    public function search(MedecinRepository $medecinRepository): Response
    {
        $request = Request::createFromGlobals();
        $request->query->get("nom");
        $query = $request->request->get('nom');
        if($query == 'nom') {
            $medecins = $medecinRepository->findMedecinByNom($query);
        } else {
            $medecins = $medecinRepository->findAll();
...
// TODO $request = Request::createFromGlobals(); $request->query->get("nom");
